...
export const AiStudioPlayground: React.FC<AiStudioPlaygroundProps> = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome-msg',
      role: 'assistant',
      content: 'Olá! Sou a Professora Gabi, sua assistente de estudos do CFJVMG! Como posso te ajudar hoje? Envie dúvidas, redações, arquivos ou perguntas para estudarmos juntos.',
  ...
  const handleExportHistory = () => {
    ...
    let exportContent = `=== HISTÓRICO DE CONVERSA - TIRA-DÚVIDAS CFJVMG AI ===\n`;
    exportContent += `Data de Exportação: ${new Date().toLocaleString('pt-BR')}\n`;
    exportContent += `Modelo: ${model} | Temperatura: ${temperature}\n\n`;

    messages.forEach((msg) => {
      const author = msg.role === 'user' ? 'ESTUDANTE' : 'PROFESSORA GABI (CFJVMG AI)';
    ...
    a.href = url;
    a.download = `historico_tira_duvidas_cfjvmg_${new Date().toISOString().slice(0, 10)}.txt`;
    ...
    setMessages([
      {
        ...
        role: 'assistant',
        content:
          'Histórico limpo! Sou a Professora Gabi, sua assistente de estudos do CFJVMG! Como posso te ajudar agora? Envie dúvidas, redações, arquivos ou perguntas para estudarmos juntos.',
        timestamp: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }),
      },
    ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showError(err, {
        componentName: 'Tira-Dúvidas CFJVMG AI',
        retryAction: () => {
    ...
    <div className="flex flex-col h-full min-h-[600px] max-h-[85vh] bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl text-slate-100 relative">
      ...
      {showClearConfirm && (
        <div className="absolute inset-0 z-40 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 max-w-sm w-full text-center shadow-2xl">
            ...
            <p className="text-xs text-slate-400 mb-4">Essa ação apagará todas as mensagens atuais da conversa com a Professora Gabi.</p>
      ...
      <div className="flex items-center justify-between px-5 py-3.5 bg-slate-900 border-b border-slate-800">
        <div className="flex items-center gap-3">
          <GabiAvatar
            ...
            statusBadgeSize={10}
            alt="Professora Gabi - CFJVMG AI"
          />
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-bold text-white">Tira-Dúvidas CFJVMG AI</h2>
            ...
            <p className="text-xs text-slate-400">Tutora inteligente do CFJVMG com texto, imagens, redação e voz</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {hasSettingsHandler && (
            <button
              ...
              title="Configurações de IA e Modelo"
          ...
          <div className="flex items-center gap-1.5 truncate">
            ...
            <span className="truncate">
              ...
              {apiKey ? (
                ...
                <span className="text-slate-400 font-mono">Padrão do CFJVMG (ou insira sua chave nas preferências)</span>
              )}
            </span>
          </div>
          <button
            type="button"
            onClick={handleTriggerSettings}
            className="text-[11px] text-purple-400 hover:text-purple-300 underline underline-offset-2 shrink-0 ml-2 cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
        {messages.map((msg) => (
          ...
          >
            {msg.role === 'assistant' ? (
              <GabiAvatar size={34} showOnlineStatus={false} alt="Professora Gabi" />
            ) : (
              <div className="w-8 h-8 rounded-full bg-amber-500 text-slate-950 flex items-center justify-center shrink-0 text-xs font-bold shadow-sm">
            ...
            <div
              className={`group relative rounded-2xl px-4 py-3 text-sm leading-relaxed max-w-[85%] sm:max-w-[75%] border break-words ${
                msg.role === 'user'
                  ? 'bg-purple-600 text-white border-purple-500/50 rounded-tr-none'
        ...
        {isLoading && (
          <div className="flex gap-3 max-w-3xl mr-auto items-start">
            <GabiAvatar size={34} showOnlineStatus={true} statusBadgeSize={8} alt="Professora Gabi" />
            <div className="bg-slate-900 border border-slate-800 rounded-2xl rounded-tl-none px-4 py-3 text-sm text-slate-300 flex items-center gap-2.5 shadow-md">
              <Loader2 className="w-4 h-4 animate-spin text-purple-400" />
              <span>Professora Gabi está analisando e formulando a resposta...</span>
            </div>
          </div>
        )}
      ...
      <div className="p-4 bg-slate-900 border-t border-slate-800 space-y-3">
        ...
        <form onSubmit={handleSendMessage} className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2.5 text-slate-400 hover:text-purple-400 hover:bg-slate-800 rounded-xl transition-colors border border-slate-700 shrink-0 cursor-pointer"
          >
          ...
          <input
            ...
            placeholder="Envie sua dúvida, questão ou redação para a Professora Gabi..."
            disabled={isLoading}
            className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
          />

          <button
            type="submit"
            disabled={isLoading || (!inputPrompt.trim() && attachments.length === 0)}
            className="p-2.5 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:hover:bg-purple-600 text-white rounded-xl transition-all shadow-md shadow-purple-600/20 shrink-0 cursor-pointer"
          >

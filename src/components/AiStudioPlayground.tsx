...
export const AiStudioPlayground: React.FC<AiStudioPlaygroundProps> = ({
  ...
  const handleExportHistory = () => {
    ...
    let exportContent = `=== HISTÓRICO DE CONVERSA - TIRA-DÚVIDAS GABARITOU AI ===\n`;
    exportContent += `Data de Exportação: ${new Date().toLocaleString('pt-BR')}\n`;
    exportContent += `Modelo: ${model} | Temperatura: ${temperature}\n\n`;

    messages.forEach((msg) => {
      const author = msg.role === 'user' ? 'ESTUDANTE' : 'PROFESSORA GABI (GABARITOU AI)';
    ...
    a.href = url;
    a.download = `historico_tira_duvidas_gabaritou_${new Date().toISOString().slice(0, 10)}.txt`;

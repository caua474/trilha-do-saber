...
export const SocialShareStoryModal: React.FC<SocialShareStoryModalProps> = ({
  ...
  const handleCopyText = () => {
    ...
    let text = '';
    if (type === 'redacao') {
      text = `🔥 Tirei ${data.score || 920}/1000 na Redação ENEM treinando no MenteUp! 🚀 Rumo à aprovação em ${data.courseTarget || 'Medicina'}! #MenteUp #ENEM2026 #Redacao1000`;
    } else if (type === 'mascote') {
      text = `⭐ Meu mascote ${data.mascotName || 'Gabaritão'} atingiu o Nível ${data.mascotLevel || 5} com ${data.mascotXp || 1200} XP no MenteUp! Rumo ao ENEM! 🚀 #MenteUp #Estudos`;
    } else if (type === 'quiz') {
      text = `⚡ Gabaritei o Quiz Rápido da Professora Gabi no MenteUp! 🎯 ${data.quizScore || 3}/${data.quizTotal || 3} acertos (${data.quizPercent || 100}%) no tema "${data.quizTopic || 'Revisão Rápida ENEM'}". Rumo à nota máxima no ENEM! 🚀 #MenteUp #ENEM2026 #ProfessoraGabi #QuizENEM`;
    } else {
      text = `🔥 Consegui ${data.streakDays || 14} Dias Seguidos de Ofensiva Diária no MenteUp! Foco total na aprovação! 🎓 #Streak #MenteUp #FocoENEM`;
    }
  ...
  const getShareText = () => {
    if (type === 'redacao') {
      return `🔥 Tirei ${data.score || 920}/1000 na Redação ENEM treinando no MenteUp! 🚀 Rumo à aprovação em ${data.courseTarget || 'Medicina'}! #MenteUp #ENEM2026 #Redacao1000`;
    } else if (type === 'mascote') {
      return `⭐ Meu mascote ${data.mascotName || 'Gabaritão'} atingiu o Nível ${data.mascotLevel || 5} com ${data.mascotXp || 1200} XP no MenteUp! Rumo ao ENEM! 🚀 #MenteUp #Estudos`;
    } else if (type === 'quiz') {
      return `⚡ Gabaritei o Quiz Rápido da Professora Gabi no MenteUp! 🎯 ${data.quizScore || 3}/${data.quizTotal || 3} acertos (${data.quizPercent || 100}%) no tema "${data.quizTopic || 'Revisão Rápida ENEM'}". Rumo à nota máxima no ENEM! 🚀 #MenteUp #ENEM2026 #ProfessoraGabi #QuizENEM`;
    } else {
      return `🔥 Consegui ${data.streakDays || 14} Dias Seguidos de Ofensiva Diária no MenteUp! Foco total na aprovação! 🎓 #Streak #MenteUp #FocoENEM`;
    }
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Meu Desempenho no MenteUp',
          text: getShareText(),
